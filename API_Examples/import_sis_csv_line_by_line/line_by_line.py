...
DOMAIN = 'your_canvas_domain.instructure.com'
...
def checkToken(): return requests.get(base_url.format(''),headers=header).status_code == 200
...
if __name__ == '__main__':
  ...
  with open(sys.argv[1],'rU') as _f:
    ...
    with progress.Bar(label='importing status',expected_size=len(csv_r)) as bar :
      ...
      # processing_errors
      for r in filter(lambda i: i.has_key('processing_errors'),finished_imports):
        print r
